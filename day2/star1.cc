...
int convertRPSCode(string code)
{
   ...
   map.insert(pair<string, int>("X", 0));
   map.insert(pair<string, int>("Y", 1));
   map.insert(pair<string, int>("Z", 2));
...
int main()
{
    ...
    while(!reader.eof())
    {
        ...
        cout << rps_round << endl;
        if (rps_round != "") {
            opponent_moves.push_back(convertRPSCode(rps_round.substr(0,1)));
            my_moves.push_back(convertRPSCode(rps_round.substr(2,1)));
        }
    }
