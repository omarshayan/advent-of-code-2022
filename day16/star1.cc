...
int run(map<string, int> flowrates, map<string,map<string,int>> dists, string curr, map<string, bool> opened, int minleft){
    if (minleft <=0) {
        /*
    ...
    for (auto adj : dists[curr]) {
        string v = adj.first;
        if(!opened[v] && flowrates[v] > 0){
            map<string, bool> ocp(opened);
            ocp[v] = true;
            if(minleft ==30){cout << "on iter " << kek << "\t out of " << dists[curr].size() << endl; kek++;}
            //cout << "opening " << v << endl;
            best = max(best, score + run(flowrates, dists, adj.first, ocp, minleft - adj.second - 1));
...
int main() {
    ...
    int res = run(flowrates, dists, "AA", o, 30);
    cout << "res: " << res <<endl;
}
