...
int traverse(Node* dir, int& sum, int& bestdirsum, int neededspace) {
    if (dir == NULL)
        return sum;
...
//        cout << "traversing " << child -> name << endl;
        dirsum += traverse(child, sum, bestdirsum, neededspace);
    }
    

    sum += dirsum;
    if (dirsum <= bestdirsum && dirsum > neededspace) { 
...
int main() {
    ...
    sum = traverse(root, sum, smalldirsum, 0);
    cout << "sum: " << sum << endl;

    int diskspace = 70000000;
    int freespace = diskspace - sum;
    int neededspace = 30000000 - freespace;

    sum = 0;
    int bestdirspace = diskspace;
    sum = traverse(root, sum, bestdirspace, neededspace);
    
    cout << "sum: " << sum << endl;
    
    cout << "bestdirspace: " << bestdirspace << endl;
